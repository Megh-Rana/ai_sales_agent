...
export const CalendlyFollowUpsSection: React.FC = () => {
  ...
  return (
    <div className="bg-[#121620] border border-[#232B3B] rounded-2xl overflow-hidden shadow-xl mb-8">
      {/* Header */}
      <div className="p-6 border-b border-[#232B3B] bg-[#151B28] flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30 font-semibold uppercase">
                24h Window Active
              </span>
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="p-2 rounded-lg bg-[#1E2536] hover:bg-[#252E42] border border-[#2D374D] text-slate-300 hover:text-white transition-colors disabled:opacity-50"
      ...
      <div className="p-6">
        {trackings.length === 0 ? (
          <div className="text-center py-10 bg-[#0E121A] border border-[#1E2536] rounded-xl">
            <Calendar className="w-10 h-10 text-slate-600 mx-auto mb-3" />
            <h4 className="text-sm font-semibold text-slate-300 mb-1">No Calendly Links Dispatched Yet</h4>
                ...
                <div 
                  ...
                  className={`p-4 rounded-xl border transition-all ${
                    isBooked 
                      ...
                      : 'bg-[#151A25] border-[#232B3B] hover:border-slate-600'
                  }`}
                >
                  {/* Top Row: Lead & Status */}
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      <h4 className="text-sm font-bold text-white">{t.lead_name}</h4>
                      <p className="text-xs text-slate-400">{t.company_name}</p>
                    </div>

                    {isBooked ? (
                      <span className="flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/40">
                        <CheckCircle2 className="w-3 h-3" />
                        Booked
                      </span>
                    ) : isExpired ? (
                      <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-slate-500/20 text-slate-400 border border-slate-500/40">
                        Max Retries
                      </span>
                    ) : isRecalled ? (
                      <span className="flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/40">
                        <PhoneCall className="w-3 h-3" />
                        Attempt {t.retry_count}/{t.max_retries}
                      </span>
                    ) : (
                      <span className="flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/40">
                  ...
                  <div className="space-y-1.5 py-2 my-2 border-y border-[#232B3B]/60 text-xs">
                    <div className="flex items-center justify-between text-slate-300">
                      <span className="text-slate-400">Phone:</span>
                      <span className="font-mono text-slate-200">{t.phone_number}</span>
                    </div>

                    <div className="flex items-center justify-between">
                      <span className="text-slate-400">Link Opened:</span>
                      <span className={`flex items-center gap-1 text-[11px] font-medium ${
                        t.link_clicked ? 'text-emerald-400' : 'text-slate-500'
                      }`}>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400">Invite Delivery:</span>
                    ...
                    {t.email && (
                      <div className="flex items-center justify-between text-slate-300">
                        ...
                        <span className="font-mono text-[11px] text-slate-200 truncate max-w-[170px]" title={t.email}>
                          {t.email}
                        </span>
                      </div>
                    )}
                  ...
                  <div className="flex items-center justify-between gap-2 mt-3 pt-1">
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-[#1E2536] hover:bg-[#252E42] border border-[#2D374D] text-slate-300 hover:text-white transition-colors"
                    ...
                    <div className="flex items-center gap-1.5">
                      {!isBooked && (
                        <>
                          <button
                            ...
                            className="px-2.5 py-1 text-[11px] font-semibold rounded-lg bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/40 transition-colors disabled:opacity-50"
                      ...
                      {isBooked && (
                        <div className="flex items-center gap-1 text-[11px] font-medium text-emerald-400 py-1">
