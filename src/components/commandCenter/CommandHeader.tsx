      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-mono uppercase tracking-wider text-amber-400 font-semibold">
              {t.navigation.aiSalesPlatform || 'Vidur AI Sales Platform'}
            </span>
          </div>

          <h1 className="text-h2 font-bold text-foreground tracking-tight flex items-center gap-3">
            <span>{t.commandCenter?.title || 'Revenue Command Center'}</span>
            <span className="text-sm font-mono font-medium px-2.5 py-0.5 rounded-full bg-surface-1 border border-border-subtle text-foreground-secondary">
              {t.commandCenter?.liveOrchestration || 'Live Orchestration'}
            </span>
          </h1>

          <p className="text-small text-foreground-tertiary mt-0.5 flex items-center gap-2">
            <span className="text-amber-400 font-semibold">{(metrics?.urgentActionCount || 0)} {t.commandCenter?.priorityActionsRequireFocus || 'priority actions require focus'}</span>
            <span className="text-border-subtle">•</span>
            <span>{metrics?.activePipelineValue || '₹0'} {t.commandCenter?.activePipelineValue || 'active pipeline value'}</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          >
            ...
            <span>{isRefreshing ? (t.commandCenter?.refreshingTelemetry || 'Refreshing Telemetry...') : (t.commandCenter?.refreshTelemetry || 'Refresh Telemetry')}</span>
          </button>
        </div>
      </div>
    </div>
