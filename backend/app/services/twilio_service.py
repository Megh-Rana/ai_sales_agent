...
class TwilioService:
    ...
    def generate_audio_greeting_twiml(
        ...
        """
        Plays the opening pitch audio then immediately starts a <Record> to capture
        the caller's response. Twilio POSTs the recording URL to gather_action_url
        (our /twilio/gather webhook), where we run our own Sarvam STT instead of
        relying on Twilio's built-in speech recognition (which is unreliable on
        Indian PSTN numbers with trial accounts).
        """
        if TWILIO_AVAILABLE:
            resp = VoiceResponse()
            resp.play(audio_url)
...
<Response>
    <Play>{audio_url}</Play>
    <Record action="{gather_action_url}" method="POST" maxLength="15" timeout="10" playBeep="false" finishOnKey=""/>
    <Hangup/>
</Response>"""
        return f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
    ...
    def generate_audio_response_twiml(
        ...
        """
        Plays the AI response audio then starts a <Record> to capture the caller's
        next utterance for our own Sarvam STT. Falls back to hangup when requested.
        """
        if TWILIO_AVAILABLE:
            resp = VoiceResponse()
            resp.play(audio_url)
...
<Response>
    <Play>{audio_url}</Play>
    <Record action="{next_gather_url}" method="POST" maxLength="15" timeout="10" playBeep="false" finishOnKey=""/>
    <Hangup/>
</Response>"""
        return f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
...
<Response>
    ...
    @classmethod
    def generate_filler_redirect_twiml(cls, filler_url: str, redirect_url: str) -> str:
        """
        Generates TwiML that plays a short filler audio (e.g. "One moment...") and then
        immediately <Redirect>s to the given URL. Used to return an instant response to
        Twilio while the LLM + TTS background task finishes in parallel — preventing the
        5-second webhook timeout from terminating the call.
        """
        if TWILIO_AVAILABLE:
            resp = VoiceResponse()
