...
class TwilioService:
    ...
    def generate_audio_greeting_twiml(
        ...
        Nest <Play> inside <Gather> to support instant prospect speech barge-in.
        """
        if TWILIO_AVAILABLE:
            resp = VoiceResponse()
...
<Response>{gather_tag}
    ...
    def generate_audio_response_twiml(
        ...
        if hangup:
            ...
            body = f'\n    <Gather input="speech" action="{next_gather_url}" method="POST" speechTimeout="auto" timeout="5">\n        <Play>{audio_url}</Play>\n    </Gather>\n    <Hangup/>'
        else:
            body = f'\n    <Play>{audio_url}</Play>'
