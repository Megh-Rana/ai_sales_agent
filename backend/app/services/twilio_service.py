...
class TwilioService:
    ...
    def get_credentials(cls) -> Dict[str, Optional[str]]:
        """Retrieves and normalizes Twilio environment credentials."""
        account_sid = os.getenv("TWILIO_ACCOUNT_SID", "").strip()
        auth_token = os.getenv("TWILIO_AUTH_TOKEN", "").strip()
        phone_number = os.getenv("TWILIO_PHONE_NUMBER", "").strip()
        webhook_base = os.getenv("TWILIO_WEBHOOK_BASE_URL", "").strip() or os.getenv("VITE_API_URL", "").strip()
    ...
    def make_outbound_call(
        ...
        base_url = webhook_base_url or creds.get("webhook_base_url") or "http://localhost:8000"
    ...
    def generate_audio_greeting_twiml(
        ...
        gather_tag = f'\n    <Gather input="speech" action="{gather_action_url}" method="POST" speechTimeout="auto" timeout="5"/>\n    <Hangup/>' if gather_action_url else ""
        return f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Play>{audio_url}</Play>{gather_tag}
</Response>"""
    ...
    def generate_audio_response_twiml(
        ...
        gather_tag = f'\n    <Gather input="speech" action="{next_gather_url}" method="POST" speechTimeout="auto" timeout="5"/>' if not hangup and next_gather_url else ""
        return f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Play>{audio_url}</Play>{hangup_tag}{gather_tag}
</Response>"""
